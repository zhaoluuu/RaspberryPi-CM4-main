...
from uiutils import (
    clear_bottom,clear_top,lcd_draw_string, display, splash, 
...
def QwenVL_api_picture(PROMPT='追踪积木块上方的物体'):
    base64_image = encode_image("./demos/speech_AI_food/myrec.jpeg")
    client = OpenAI(
...
result_my = QwenVL_api_picture(message)

# 让用户选择 ROI
bbox = (result_my[0], result_my[1], result_my[2]-result_my[0], result_my[3]-result_my[1])  #替换成大模型给的坐标
...
while True:
    ...
    if success:
        ...
        # 跟踪失败时，尝试重新选择目标
        # bbox = cv2.selectROI("请重新框选跟踪目标，按 Enter 确认", frame, showCrosshair=False)
        # if bbox[2] > 0 and bbox[3] > 0:  # 确保新选择的区域有效
        #     tracker = cv2.TrackerCSRT_create()
        #     tracker.init(frame, bbox)
        # else:
        #     break  # 如果用户未选择有效区域，退出程序
