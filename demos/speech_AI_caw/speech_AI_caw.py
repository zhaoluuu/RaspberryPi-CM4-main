...
def open_AI_play():
    ...
    if la == 'cn':
        text_lines = [
            ("抓取红色小球/方块|拿取红色方块", 150),
            ("抓取绿色小球/方块|拿取绿色方块", 170),
            ("抓取蓝色小球/方块|拿取蓝色方块", 190),
            ("抓取黄色小球/方块|拿取黄色方块", 210),
        ]
    else:
       text_lines = [
            ("Take the red square", 150),
            ("Take the green square" , 170),
            ("Take the blue square", 190),
            ("Take the yellow square",210)
...
def actions_AI(act):
    ...
    if mark:
        ...
        if mincmd==1 or mincmd==6:
            time.sleep(1)
            os.system("python3 ./demos/speech_AI_caw/speach_caw.py red") 
            os._exit(0) #退出此程序 ,自动也会退出

        elif mincmd==2 or mincmd==7:
            time.sleep(1)
            os.system("python3 ./demos/speech_AI_caw/speach_caw.py green")
            os._exit(0) #退出此程序 ,自动也会退出

        elif mincmd==3 or mincmd == 8:
            time.sleep(1)
            os.system("python3 ./demos/speech_AI_caw/speach_caw.py blue")
            os._exit(0) #退出此程序 ,自动也会退出

        elif mincmd==4 or mincmd==9:
            time.sleep(1)
            os.system("python3 ./demos/speech_AI_caw/speach_caw.py yellow")
            os._exit(0) #退出此程序 ,自动也会退出

        elif mincmd==5 or mincmd == 10: #调用拍照视觉！
            time.sleep(1)
            os.system("python3 ./demos/speech_AI_caw/speech_picture.py "+act)
            os._exit(0) #退出此程序 ,自动也会退出
